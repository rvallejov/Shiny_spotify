...
shinyServer(function(input, output){
  ...
  output$distinct.artists <- renderInfoBox({
    infoBox("Distinct artists", 
            comma(n_distinct(spotify.data$Artist)), 
            icon = icon("music"),
            width = "auto",
            color = "red",
            fill = TRUE)
  ...
  output$distinct.tracks <- renderInfoBox({
    infoBox("Distinct tracks", 
            comma(n_distinct(spotify.data$Track.Name)), 
  ...
  output$total.streams <- renderInfoBox({
    infoBox("Total streams", 
            comma(sum(as.numeric(spotify.data$Streams[which(spotify.data$Region != "global")]))), 
            icon = icon("globe"),
            width = "auto",
            color = "green",
            fill = TRUE)
  ...
  output$scatter2 <- renderPlotly({
    g2 <- top.ten.region %>%
      ...
      geom_vline(aes(xintercept = mean(top.ten.region$mean.days.to.top)),
                 color = "steel blue", linetype = 2, alpha = 0.7) +
      geom_point(aes(text=sprintf("Continent: %s<br>Streams: %s<br>Days to top: %s<br>Days after top: %s", 
                                  Continent, comma(total.streams), round(mean.days.to.top), round(mean.days.after.top)),
  ...
  output$scatter1 <- renderPlotly({
    g3 <- randomVals() %>%
      ...
      geom_point(aes(text=sprintf("Track: %s<br>Streams: %s",
                                  Track.Name, comma(Streams)),
